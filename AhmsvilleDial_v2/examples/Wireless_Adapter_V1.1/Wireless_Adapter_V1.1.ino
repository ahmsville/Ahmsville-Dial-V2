...
struct W_adapterPacket {
  byte appID = 0; //from PC via SerialUSB, is also ack payload
...
RF24 radio(6, 7);                          // Set up nRF24L01 radio on SPI bus plus(CE,CSN)
...
int radiofailcheckinterval = 500;
long radiofailchecktimer = 0;
...
void loop() {
  ...
  if (millis() - radiofailchecktimer > radiofailcheckinterval) {
    radiofailchecktimer = millis();
  ...
  receiveDataFromDial();
  //SerialUSB.println(dialPacket.spacenavRawAvailable);
  /*
...
void connecttoPC() {
  ...
  if (inString != "") {
    ...
    if (inString != "a") { //change app byte
      ...
      digitalWrite(4, HIGH);
      ledofftime = millis();
      SerialUSB.print("**************Wireless Adapter"); //reply with your dial type
  ...
  if ((millis() - stillavailable) > 5000) { //update wirelessly connected dial to pc every few secs
    if (dialPacket.dialID == 1) {
      SerialUSB.print("*******************|||1|0|0|0|");
    }
    else if (dialPacket.dialID == 2) {
      SerialUSB.print("*******************|||0|1|0|0|");
    }
    else if (dialPacket.dialID == 3) {
      SerialUSB.print("*******************|||0|0|1|0|");
    }
    else if (dialPacket.dialID == 4) {
      SerialUSB.print("*******************|||0|0|0|1|");
    }
...
void receiveDataFromDial() {
  ...
  if (dialPacket.dialID != 0) {
    uint32_t failtimeout = millis();
    while (radio.available()) {
      digitalWrite(4, HIGH);
      if (millis() - failtimeout > 500) {
        radio.failureDetected = true;
        digitalWrite(4, LOW);
        //SerialUSB.println("Radio available failure detected");
        break;
      }
      ledofftime = millis();
  ...
  if (dialPacket.keystroke != '0' || dialPacket.mousecontrol != '0') { //is mouse or key input
    ...
    if (dialPacket.inapp) { //is inapp command
      ...
      if (dialPacket.inappcommandID < 10) {
        SerialUSB.print(sss + "the app is in charge of ths00" + dialPacket.inappcommandID); //send inappcommandID to pc
      }
      else if (dialPacket.inappcommandID >= 10 && dialPacket.inappcommandID <= 99) {
        SerialUSB.print(sss + "the app is in charge of ths0" + dialPacket.inappcommandID); //send inappcommandID to pc
      }
      else if (dialPacket.inappcommandID >= 100) {
        if (dialPacket.inappcommandID == 222) {
          WAPacket.appID = 0;
        }
        SerialUSB.print(sss + "the app is in charge of ths" + dialPacket.inappcommandID); //send inappcommandID to pc
      }
