...
private:
	...
	int haptics_pin = 0, haptics_duration = 100, haptics_strength = 255, haptics_ontime, haptics_offtime, haptics_state;
	float hresknob1alpha = 0.1, prevHRknob1 = 0.0;
	float hresknob2alpha = 0.1, prevHRknob2 = 0.0;
...
public:
	...
	void setknobresolution(int res_percent, int knobnum, bool abs);
	void normalize();
	void normalizeCustom(int pad);
	int get_knobState(int sensornum);
	void setsleep(bool slpact);
	bool readsleep();
	void eventnotification(int state);
	float knobAbsolute();
